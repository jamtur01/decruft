...
pub fn try_extract(
    ...
    if let Some(result) = github::extract_github(html, url, include_replies, allow_network) {
        return Some((result, "github"));
    }
    if let Some(result) = reddit::extract_reddit(html, url, include_replies) {
        return Some((result, "reddit"));
    }
    if let Some(result) = hackernews::extract_hackernews(html, url, include_replies, allow_network)
    {
        return Some((result, "hackernews"));
    }
    if let Some(result) =
        stackoverflow::extract_stackoverflow(html, url, include_replies, allow_network)
    {
        return Some((result, "stackoverflow"));
    }
    if let Some(result) = lobsters::extract_lobsters(html, url, include_replies, allow_network) {
        return Some((result, "lobsters"));
    }
    if let Some(result) = twitter::extract_x_article(html, url, allow_network) {
        return Some((result, "twitter"));
    }
    if let Some(result) = c2wiki::extract_c2wiki(html, url, allow_network) {
